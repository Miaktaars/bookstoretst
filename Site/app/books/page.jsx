...
export default function BookListPage() {
  ...
  const [total, setTotal] = useState(0); // Keep track of the total count
  const [loading, setLoading] = useState(true);
  const [showNoCover, setShowNoCover] = useState(false); // Handle the "Don't Judge a Book by Its Cover" button
  ...
  const PAGE_SIZE = 56; // Display 56 books per page
  const totalPages = Math.ceil(total / PAGE_SIZE); // Calculate the total number of pages
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      ...
      try {
        ...
        setBooks(data.books);
        setTotal(data.total); // Set total count of books
      } catch (err) {
      ...
      <aside className="w-72 p-6 bg-black/80 backdrop-blur-md text-white sticky top-0 h-screen overflow-auto">
        <div className="mb-6 space-y-4">
          <div>
            ...
            <p className="text-sm mt-2">{total} books found</p>{" "}
            {/* Count of books */}
          </div>
          ...
          <div className="mt-6">
            <button
              ...
              onClick={() => {
                setShowNoCover((prev) => !prev);
                setPage(1); // trigger reload when the button is toggled
              }}
            >
        ...
        {loading ? (
          ...
          >
            {books.map((book) => (
              <BookCard
                key={book._id}
                book={showNoCover ? { ...book, image: "/no_cover.webp" } : book} // Toggling the cover image
              />
            ))}
          </div>
        )}
